...
while b >= 1:
    ...
    r=compute_r(u,v,f)
    q=compute_q(u,v,f)
    strength = compute_strength(q,nav,len(tree_ls))
    correlation, rho = compute_correlation(u,v,f,nav,len(tree_ls))
    
    accuracy = compute_accuracy(strength, correlation)
    change_in_u, change_in_v = compute_qu_qv(u, v, f)
    
    nu=compute_nu(q, rho , nav, len(tree_ls))
    
    l= compute_l(q, nav, b)
    
    dletaB = compute_deltaB(change_in_u, change_in_v, del_u, del_v, l, nu)
